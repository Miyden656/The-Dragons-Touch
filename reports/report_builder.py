...
def build_normal_report(context: dict[str, Any]) -> str:
    ...
    lines.extend(_format_cut_entries(possible.optional_cut_candidates, "No optional optimization candidates in this checkpoint."))
    ...
    return _v09551_force_need_aligned_addendum_after_exact_preview(_v0954_add_budget_bracket_labels_to_exact_preview(_v0953_rebuild_exact_preview_with_color_guard(_v0952_insert_exact_full_pool_preview(_v0951_replace_full_pool_preview_section(_v094610_suppress_dragon_gate_from_strong_owned_text("\n".join(lines)), context))))).rstrip() + "\n"
...
def _v095552_final_report_postprocess(report_text: str) -> str:
    report_text = _v095552_clean_fallback_trigger_lines(report_text)
    report_text = _v095552_add_need_aligned_addendum(report_text)
    return report_text


if "build_normal_report" in globals() and "_v095552_original_build_normal_report" not in globals():
    _v095552_original_build_normal_report = build_normal_report

    def build_normal_report(context):
        return _v095552_final_report_postprocess(_v095552_original_build_normal_report(context))
