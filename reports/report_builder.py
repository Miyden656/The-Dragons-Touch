...
from philosophy.report_section import format_persona_opening_from_runtime_config  # v1.1.20 player-facing persona opening
...
def _v1120_build_persona_opening(context: dict[str, Any]) -> str:
    """Render the player-facing persona opening greeting (v1.1.20).

    Presentation-only. Falls back to a neutral, still-pilot-facing note if no
    philosophy context is available or the formatter cannot render, so the opening
    is never load-bearing and never reintroduces developer/version framing.
    """
    fallback = (
        "> This is a review summary. Treat every recommendation as guidance and keep "
        "your own stated intent as the final authority."
    )
    philosophy_context = context.get("philosophy_context")
    if not philosophy_context:
        return fallback
    try:
        runtime_philosophy_config = _v1118_runtime_config_from_legacy_philosophy_context(context)
        opening = format_persona_opening_from_runtime_config(runtime_philosophy_config).rstrip()
        return opening or fallback
    except Exception:
        return fallback
