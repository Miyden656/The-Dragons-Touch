...
APP_VERSION = "v0.6.7.9.12"
APP_PHASE = "Review Setup Boundary Cleanup and Commander Pair Preview Fix"
...
class MainWindow(QMainWindow):
    ...
    def summarize_deck_preview(self, text, fallback_name):
        ...
        v0.6.7.9.12 improves the preview estimate by counting commander-section
        cards and collecting multiple commander lines. This is not legality
        validation; it only prevents partner/background-style decks from looking
        under-counted in the UI.
        """
        section = "main"
        commander_names = []
        main_count = 0
        ...
        if commander_detected:
            # Preserve readable pairing without requiring the UI preview to validate legality.
            deduped = []
            for name in commander_names:
                if name not in deduped:
                    deduped.append(name)
            commander = " / ".join(deduped)
        else:
        ...
        if total_count == 100 and commander_detected:
            note = f"Preview loaded. Estimate: {main_count} main-deck card(s) + {commander_count} commander card(s) = 100 total. Backend validation still required."
        elif main_count == 0:
            ...
            note = f"Preview loaded. Estimate: {main_count} main-deck card(s) + {commander_count} commander card(s) = {total_count} total. Backend validation still required."
