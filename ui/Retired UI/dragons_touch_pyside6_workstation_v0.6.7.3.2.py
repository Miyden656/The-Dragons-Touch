...
APP_VERSION = "v0.6.7.3.2"
APP_PHASE = "Review Setup Polish and Deck Preview Spacing Cleanup"
...
# - v0.6.7.3.2 cleans up dropdown readability, default labels, and review setup handoff wording.
# - v0.6.7.3.2 polishes Review Setup dropdown popups, default-note contrast, and deck preview spacing.
...
class MainWindow(QMainWindow):
    ...
    def page_analysis_setup(self):
        page, layout = self.page_container(
            "Review Setup",
            f"Stage the same review choices the CLI already supports. {APP_VERSION} polishes dropdowns, default notes, collection handoff, and deck preview spacing before backend mapping."
        )
    ...
    def page_settings(self):
        ...
        version = ReportCard("App Version", self.theme); version.body.addWidget(self.make_text(f"The Dragon’s Touch PySide6 Workstation\nVersion: {APP_VERSION}\nPhase: {APP_PHASE}\nLocked backend: {LOCKED_BACKEND_VERSION}\nBackend: Not connected in UI yet\nPurpose: local deck-file selection plus review-settings form staging, dropdown polish, default labels, collection handoff clarity, and deck preview spacing cleanup", paper=True)); b_layout.addWidget(version); b_layout.addStretch(1); layout.addWidget(body, stretch=1); return page
