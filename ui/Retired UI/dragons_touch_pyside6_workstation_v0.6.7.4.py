...
Version: v0.6.7.4 — Collection Source Selection UI
...
APP_VERSION = "v0.6.7.4"
APP_PHASE = "Collection Source Selection UI"
...
class MainWindow(QMainWindow):
    ...
    def collection_settings_summary_text(self):
        if self.state.collection_source_mode == "Select collection files":
            ...
            if self.state.selected_collection_files:
                examples = "\n".join(f"  - {Path(path).name}" for path in self.state.selected_collection_files[:6])
                if len(self.state.selected_collection_files) > 6:
                    examples += f"\n  - ...and {len(self.state.selected_collection_files) - 6} more"
                source_detail += f"\n{examples}"
        else:
    ...
    def page_collection_tools(self):
        page, layout = self.page_container(
            "Collection Source",
            f"Stage collection behavior for future recommendations. {APP_VERSION} saves collection choices locally but does not load owned cards yet."
        )
        ...
        mode_card.body.addWidget(self.make_text(
            "Choose how The Dragon’s Touch should treat your owned cards once backend hooks are connected.",
            paper=True
        ))
        ...
        mode_card.body.addWidget(self.make_text(
            "Collection only should still be honest: if no owned card is a real fit, the report should say so instead of forcing a weak recommendation.",
            paper=True
        ))
        ...
        current_source.body.addWidget(self.make_text(self.collection_settings_summary_text(), paper=True))
        ...
        s_layout.addWidget(self.make_text(self.collection_settings_summary_text()))
