...
Version: v0.6.7.9.6 — Guide Presentation UI Field and CLI Bridge
...
APP_VERSION = "v0.6.7.9.6"
APP_PHASE = "Guide Presentation UI Field and CLI Bridge"
BACKEND_STATUS = "Guarded backend bridge available — output, review direction, build-up, prompt mode, philosophy, and guide presentation CLI bridge active"
...
class AppState:
    ...
    cli_input_bridge_scope: str = "Output mode, review direction, build-up mode when applicable, prompt mode when safe, top-level philosophy lens when safe, and guide presentation when safe"
...
class MainWindow(QMainWindow):
    ...
    def page_analysis_setup(self):
        ...
        n_title = QLabel("v0.6.7.9.6 Boundary"); n_title.setObjectName("sectionTitle"); n_layout.addWidget(n_title)
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.6 purpose: bridge Guide Presentation after the top-level Philosophy Lens prompt while keeping the guarded Build-up flow staged and explicit"
        )
    ...
    def cli_input_bridge_preview_text(self):
        ...
        return (
            "Guide Presentation CLI Input Bridge\n"
            "- bridge_scope -> Output mode, review direction, build-up mode when needed, prompt mode when safe, top-level philosophy lens when safe, and guide presentation when safe\n"
    ...
    def guarded_execution_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.6 sends known CLI answers through top-level Philosophy Lens for the Build-up flow.\n"
            ...
            "- cli_input_bridge -> active for output mode, review direction, build-up mode when applicable, prompt mode when safe, top-level philosophy lens when safe, and guide presentation when safe\n"
    ...
    def guarded_execution_placeholder_message(self):
        QMessageBox.information(
            ...
            "v0.6.7.9.6 can attempt a guarded QProcess run of py main.py only after explicit confirmation.\n\n"
            "This patch sends Output Mode, Review Direction, Build-Up Mode when Review Direction is Build up, Prompt Mode, the top-level Philosophy Lens, and Guide Presentation when the known Build-up flow reaches those prompts, then safely captures the next prompt/error for review."
        )
    ...
    def start_guarded_backend_run(self):
        ...
        self.backend_process_timed_out = False
        self.guarded_cli_input_sent = False
        self.state.cli_input_bridge_last_sent = "Pending output-mode, review-direction, optional build-up-mode, safe prompt-mode, safe philosophy-lens, and safe guide-presentation inputs..."
        self.refresh_run_analysis_previews()
    ...
    def backend_runtime_config_boundary_text(self):
        return (
            ...
            "- This patch answers only known top-level prompts. Specific philosophy subtypes and later prompts should be mapped deliberately instead of guessed."
        )
    ...
    def page_run_review(self):
        ...
        boundary_card = ReportCard("Safety Boundary and Future Stages", self.theme, badges=[("v0.6.7.9.6", "manual")])
        stage_text = (
