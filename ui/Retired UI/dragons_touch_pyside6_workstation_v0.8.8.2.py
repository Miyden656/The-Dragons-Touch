...
class MainWindow(QMainWindow):
    ...
    def refresh_run_analysis_previews(self):
        ...
        if getattr(self, "run_analysis_content_stack", None) is not None:
            self.run_analysis_content_stack.setCurrentIndex(1 if self.state.guarded_run_in_progress else 0)
        if getattr(self, "run_analysis_running_status_label", None) is not None:
            self.run_analysis_running_status_label.setText(
                ...
                else "The forge is ready. Run Analysis will use the guarded backend and optional combo settings from Review Setup."
            )
        ...
        for button in guarded_buttons:
            ...
            button.setText("Running Analysis..." if self.state.guarded_run_in_progress else "Run Analysis")
    ...
    def finish_guarded_backend_run(self, exit_code, exit_status):
        ...
        self.refresh_run_analysis_previews()
        if exit_code == 0:
            QTimer.singleShot(120, lambda: self.go_to(self.REPORT))
        else:
