...
APP_VERSION = "v0.6.7.9.22"
APP_PHASE = "Report Viewer Readability and Run Analysis Final Cleanup"
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.report_viewer_font_label = None
    ...
    def page_run_review(self):
        page, layout = self.page_container(
            "Run Analysis",
            f"Run py main.py through guarded confirmation, then read generated reports in Report Viewer. {APP_VERSION} keeps diagnostics available but secondary."
        )
        ...
        status = QLabel("Run the forge here, then read the manuscript in Report Viewer. Diagnostics stay tucked behind the detail selector.")
        ...
        selector_note = self.default_note("Diagnostics are available here; generated report reading belongs in Report Viewer.")
    ...
    def apply_report_viewer_readability_settings(self):
        """Apply report-reader font size and wrapping without reloading the file."""
        ...
        if self.report_viewer_font_label is not None:
            self.report_viewer_font_label.setText(f"Text size: {self.state.report_viewer_font_size}")
    ...
    def change_report_viewer_font_size(self, delta):
        self.state.report_viewer_font_size = max(10, min(24, self.state.report_viewer_font_size + delta))
        self.apply_report_viewer_readability_settings()
    ...
    def page_report_viewer(self):
        page, layout = self.page_container(
            "Report Viewer",
            f"Read generated Dragon's Touch reports with grouped files, search, text sizing, and safe file controls. {APP_VERSION} keeps report parsing plain-text for now."
        )
        body = QWidget()
        ...
        smaller_btn.clicked.connect(lambda: self.change_report_viewer_font_size(-1))
        larger_btn = QPushButton("A+")
        larger_btn.clicked.connect(lambda: self.change_report_viewer_font_size(1))
        ...
        font_label = QLabel(f"Text size: {self.state.report_viewer_font_size}")
        font_label.setObjectName("defaultNote")
        self.report_viewer_font_label = font_label
        ...
        reader_tools.addWidget(smaller_btn)
        reader_tools.addWidget(larger_btn)
        reader_tools.addWidget(wrap_btn)
        reader_tools.addWidget(font_label)
