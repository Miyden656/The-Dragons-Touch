...
Version: v0.6.7.6.2 — Runtime Config Contract Polish
...
APP_VERSION = "v0.6.7.6.2"
APP_PHASE = "Runtime Config Contract Polish"
...
# - v0.6.7.7 should be the first safe backend-call bridge, if the config contract checks out.
...
class MainWindow(QMainWindow):
    ...
    def page_analysis_setup(self):
        ...
        n_title = QLabel("v0.6.7.6.2 Boundary"); n_title.setObjectName("sectionTitle"); n_layout.addWidget(n_title)
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.6.2 purpose: polish the refreshed UI mapping into a clear backend handoff contract without executing the backend"
        )
    ...
    def backend_runtime_config_mapping_text(self):
        ...
        return (
            ...
            "- next_patch_candidate -> v0.6.7.7 first safe backend bridge preview\n"
        )

    def backend_runtime_config_boundary_text(self):
        return (
            "v0.6.7.6.2 boundary\n"
            "- This page now presents the staged UI state as a backend runtime-config handoff contract.\n"
            ...
            "- v0.6.7.6.1 confirmed the mapping refreshes live. v0.6.7.6.2 clarifies what the future bridge should consume.\n"
            "- The next backend bridge patch should use this contract rather than inventing a separate workflow."
        )
    ...
    def run_placeholder_message(self):
        QMessageBox.information(
            ...
            f"{APP_VERSION} has gathered the staged UI choices into a backend runtime config contract preview. It still does not call the locked {LOCKED_BACKEND_VERSION} analysis engine. Review the Run Configuration Preview and Runtime Config Mapping Preview before a future backend bridge patch."
        )

    def page_run_review(self):
        page, layout = self.page_container(
            "Run Analysis",
            f"Review the staged UI handoff contract for a future backend runtime config. {APP_VERSION} does not call the analysis engine."
        )
        ...
        stages_card = ReportCard("Future Backend Mapping Stages", self.theme, badges=[("v0.6.7.6.2", "manual")])
        stage_text = (
        ...
        b_title = QLabel("v0.6.7.6.2 Boundary"); b_title.setObjectName("sectionTitle"); b_layout.addWidget(b_title)
    ...
    def page_settings(self):
        ...
        version = ReportCard("App Version", self.theme); version.body.addWidget(self.make_text(f"The Dragon’s Touch PySide6 Workstation\nVersion: {APP_VERSION}\nPhase: {APP_PHASE}\nLocked backend: {LOCKED_BACKEND_VERSION}\nBackend: Not connected in UI yet\nPurpose: local deck preview, review settings, collection-source staging, and backend runtime-config contract preview while keeping backend execution disconnected", paper=True)); b_layout.addWidget(version); b_layout.addStretch(1); layout.addWidget(body, stretch=1); return page
