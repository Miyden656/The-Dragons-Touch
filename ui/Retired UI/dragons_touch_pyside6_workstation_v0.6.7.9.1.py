...
Version: v0.6.7.9.1 — Interactive CLI Input Bridge Preview
...
APP_VERSION = "v0.6.7.9.1"
APP_PHASE = "Interactive CLI Input Bridge Preview"
BACKEND_STATUS = "Guarded backend bridge available — first CLI input bridge preview active"
...
class AppState:
    ...
    cli_input_bridge_scope: str = "Output mode prompt only"
...
class MainWindow(QMainWindow):
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.1 purpose: add the first controlled stdin bridge for main.py by sending only the known output-mode answer, then safely capturing the next prompt/error"
        )
    ...
    def cli_input_bridge_preview_text(self):
        ...
        return (
            "Interactive CLI Input Bridge Preview\n"
            "- bridge_scope -> Output mode prompt only\n"
            "- known_prompt -> Output mode [1=Normal]:\n"
            f"- ui_output_mode -> {self.state.output_mode}\n"
            f"- stdin_value_to_send -> {self.cli_output_mode_input_value()}\n"
            "- stdin_line_sent -> value plus newline\n"
            "- stdin_closed_after_first_answer -> True\n"
            "- additional_prompts_answered -> False\n"
            "- purpose -> advance one known CLI prompt safely, then capture the next prompt/error for mapping review\n"
    ...
    def guarded_execution_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.1 sends only the first known CLI answer: Output Mode.\n"
            "- After sending that one answer, stdin is closed so unknown later prompts are captured safely instead of guessed.\n\n"
    ...
    def guarded_execution_placeholder_message(self):
        QMessageBox.information(
            ...
            "v0.6.7.9.1 can attempt a guarded QProcess run of py main.py only after explicit confirmation.\n\n"
            "This patch sends only the first known CLI answer, the output-mode choice, then safely captures the next prompt/error for review."
        )
    ...
    def start_guarded_backend_run(self):
        ...
        message = (
            ...
            f"- The UI will send one CLI input only: output mode {self.cli_output_mode_input_value()} for {self.state.output_mode}.\n"
            "- stdin is closed after that first answer, so the next unknown interactive prompt may produce EOF and be captured safely.\n\n"
            "Run py main.py now?"
        )
        ...
        self.backend_process_timed_out = False
        self.guarded_cli_input_sent = False
        self.state.cli_input_bridge_last_sent = "Pending first output-mode input..."
        self.refresh_run_analysis_previews()
        ...
        process.start()
        QTimer.singleShot(75, self.send_first_cli_input_to_guarded_process)
        QTimer.singleShot(300000, self.timeout_guarded_backend_run)

    def send_first_cli_input_to_guarded_process(self):
        """Send only the known output-mode answer, then close stdin to avoid hanging on unknown prompts."""
        if self.backend_process is None or not self.state.guarded_run_in_progress or self.guarded_cli_input_sent:
            return
        value = self.cli_output_mode_input_value()
        self.backend_process.write(f"{value}\n".encode("utf-8"))
        self.backend_process.closeWriteChannel()
        self.guarded_cli_input_sent = True
        self.state.cli_input_bridge_last_sent = f"Sent output-mode answer {value} for UI Output Mode: {self.state.output_mode}"
        self.refresh_run_analysis_previews()
    ...
    def backend_runtime_config_boundary_text(self):
        return (
            "v0.6.7.9.1 boundary\n"
            "- This page now includes a guarded execution bridge with a first controlled stdin input bridge.\n"
    ...
    def run_placeholder_message(self):
        QMessageBox.information(
            ...
            f"{APP_VERSION} has gathered staged UI choices and can run py main.py only through the guarded confirmation button. This preview button does not run the backend. Use Guarded Execution to review the command and the first CLI input bridge before starting a confirmed run."
        )
