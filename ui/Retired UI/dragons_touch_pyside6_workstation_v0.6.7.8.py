...
Version: v0.6.7.8 — First Guarded Execution Bridge
...
APP_VERSION = "v0.6.7.8"
APP_PHASE = "First Guarded Execution Bridge"
...
class MainWindow(QMainWindow):
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.8 purpose: expose the first guarded execution bridge preview, validate the main.py entrypoint path, and show command/output/error handling plans while keeping actual execution disabled"
        )
    ...
    def backend_bridge_preview_text(self):
        ...
        return (
            ...
            "- ready_for_actual_subprocess_execution -> False"
        )
    ...
    def guarded_command_preview(self):
        """Build a visible command preview for the future guarded bridge. It is never executed here."""
        deck_path = self.state.selected_deck_path if self.state.selected_deck_path != "No deck file selected" else "<deck file required>"
        return f'python {self.state.backend_entrypoint}  # future guarded handoff deck_path="{deck_path}"'

    def guarded_execution_preview_text(self):
        """Preview the guarded execution bridge. No subprocess call is made in this patch."""
        ...
        return (
            ...
            "- This is the first guarded execution bridge layer.\n"
            "- Actual subprocess execution -> disabled in this patch.\n"
            "- main.py is validated as the intended backend entrypoint, but it is not launched here.\n"
            ...
            f"- deck_file_selected -> {deck_ready}\n"
            ...
            "- stdout_capture -> planned\n"
            "- stderr_capture -> planned\n"
            "- return_code_handling -> planned\n"
            "- timeout_handling -> planned later\n"
            "- user_readable_failure_message -> planned\n"
            "- output_folder_opening -> planned after successful run, not active yet\n\n"
            "Current Patch Safety Boundary\n"
            "- subprocess.run -> disabled\n"
            "- main.py execution -> disabled\n"
            "- backend analysis -> disabled\n"
    ...
    def guarded_execution_placeholder_message(self):
        QMessageBox.information(
            self,
            "Guarded Execution Bridge Preview",
            "v0.6.7.8 shows the guarded execution plan, validates the main.py entrypoint path, and displays the command preview.\n\n"
            "It still does not execute main.py, does not call subprocess.run, does not run backend analysis, and does not call external APIs."
        )
    ...
    def backend_runtime_config_boundary_text(self):
        return (
            "v0.6.7.8 boundary\n"
            "- This page now includes a guarded execution bridge preview while preserving the detail-selector layout.\n"
            ...
            "- The runtime-config handoff contract, safe backend bridge preview, combo tracker, and guarded execution preview remain preview-only.\n"
            ...
            "- A later actual execution patch must require explicit user action, preserve the visible command preview, capture stdout/stderr safely, handle return codes, and avoid inventing a separate workflow."
        )
    ...
    def run_placeholder_message(self):
        QMessageBox.information(
            ...
            f"{APP_VERSION} has gathered the staged UI choices into a backend bridge preview. It still does not call the locked {LOCKED_BACKEND_VERSION} analysis engine, does not run main.py, and does not execute subprocess commands. Review the runtime contract, safe bridge preview, and guarded execution preview before any future actual execution patch."
        )

    def page_run_review(self):
        page, layout = self.page_container(
            "Run Analysis",
            f"Preview the guarded main.py backend bridge, command handoff, and optional combo tracker. {APP_VERSION} validates the bridge shape but does not execute backend commands or call external APIs."
        )
        ...
        add_detail_button("Guarded Execution", 3)
        add_detail_button("Safety Boundary", 4)
        ...
        guarded_card.body.addWidget(self.default_note("This button only shows the guarded execution boundary. It does not run main.py."))
        detail_stack.addWidget(guarded_card)

        boundary_card = ReportCard("Safety Boundary and Future Stages", self.theme, badges=[("v0.6.7.8", "manual")])
        stage_text = (
