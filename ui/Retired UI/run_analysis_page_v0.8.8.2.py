...
def build_run_analysis_page(window):
    ...
    page, layout = window.page_container(
        "Run Analysis",
        f"Run the selected deck through the guarded backend. {APP_VERSION} keeps advanced diagnostics available, while the active run view hands off cleanly to Report Viewer."
    )
    ...
    run_content_stack.addWidget(run_scroll)
    ...
    running_layout.setContentsMargins(36, 36, 36, 36)
    ...
    running_orb_layout.setContentsMargins(18, 18, 18, 18)
    running_orb_layout.setSpacing(12)
    running_orb = ForgeOrb(window.theme)
    running_orb.setMinimumSize(260, 260)
    running_orb.setMaximumHeight(380)
    running_orb_layout.addWidget(running_orb, stretch=1, alignment=Qt.AlignCenter)
    running_layout.addWidget(running_orb_panel, stretch=3)

    running_status = QLabel("The helper dragon is working through the guarded backend. Report Viewer will open when the run completes successfully.")
    ...
    running_layout.addWidget(running_status)
    running_layout.addStretch(1)

    run_content_stack.addWidget(running_panel)
    run_content_stack.setCurrentIndex(1 if window.state.guarded_run_in_progress else 0)
