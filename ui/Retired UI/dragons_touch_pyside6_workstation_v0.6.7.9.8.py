...
Version: v0.6.7.9.8 — Collection Source CLI Bridge
...
APP_VERSION = "v0.6.7.9.8"
APP_PHASE = "Collection Source CLI Bridge"
BACKEND_STATUS = "Guarded backend bridge available — CLI bridge active through collection source when collection is enabled"
...
class AppState:
    ...
    cli_input_bridge_scope: str = "Output mode, review direction, build-up mode when applicable, prompt mode when safe, top-level philosophy lens when safe, guide presentation when safe, collection mode when safe, and collection source when collection is enabled"
...
class MainWindow(QMainWindow):
    ...
    def stage_review_settings(self, output_combo, direction_combo, cut_combo, build_up_combo, prompt_combo, budget_input, intended_bracket_combo, bracket_check, collection_check, summary_label=None, intensity_meaning_label=None):
    ...
    def page_analysis_setup(self):
        ...
        def auto_stage_review():
            if direction_combo.currentText() == "Auto batch":
                ...
                del blocker_build
            self.stage_review_settings(output_combo, direction_combo, cut_combo, build_up_combo, prompt_combo, budget_input, intended_bracket_combo, bracket_check, collection_check, summary_label, intensity_meaning_label)
        ...
        n_title = QLabel("v0.6.7.9.7 Boundary"); n_title.setObjectName("sectionTitle"); n_layout.addWidget(n_title)
        n_layout.addWidget(self.make_text("These choices auto-stage inside the UI as soon as you change them. Review Setup now uses one shared conditional slot: Cut down shows Review Intensity, Build up shows Build-Up Mode in that same location, and Auto batch shows both as development-only global defaults."))
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.8 purpose: bridge Collection Source after Collection Mode when collection is enabled while keeping folder/file path follow-up prompts staged for later mapping"
        )
    ...
    def cli_build_up_mode_input_value(self):
        ...
        # Fallback only: keep a safe Review Intensity route if older saved state lacks build_up_mode.
        intensity_fallback = {
            "Light": "4",
            "Normal": "3",
            "Strict": "2",
    ...
    def should_send_prompt_mode_input(self):
        """Only send prompt mode when the known CLI path has reached that prompt safely."""
        return self.state.review_direction == "Build up"

    def cli_philosophy_lens_input_value(self):
        """Map the top-level UI Philosophy Lens to the CLI philosophy prompt."""
        mapping = {
            "Balanced / Unknown": "1",
            "Timmy / Tammy": "2",
            "Johnny / Jenny": "3",
    ...
    def should_send_philosophy_lens_input(self):
        """Only send philosophy when the known Build-up CLI path has safely reached that prompt."""
    ...
    def cli_input_bridge_preview_text(self):
        ...
        philosophy_answer = self.cli_philosophy_lens_input_value() if self.should_send_philosophy_lens_input() else "not sent until prior prompts are mapped for this review direction"
        guide_answer = self.cli_guide_presentation_input_value() if self.should_send_guide_presentation_input() else "not sent until philosophy lens is safely bridged for this review direction"
        collection_answer = self.cli_collection_mode_input_value() if self.should_send_collection_mode_input() else "not sent until guide presentation is safely bridged for this review direction"
        ...
        return (
            "Collection Source CLI Input Bridge\n"
            "- bridge_scope -> Output mode, review direction, build-up mode when needed, prompt mode when safe, top-level philosophy lens when safe, guide presentation when safe, collection mode when safe, and collection source when collection is enabled\n"
            ...
            "- known_prompt_3_when_build_up -> Build-up mode [4=Finalize]:\n"
            f"- ui_build_up_mode -> {self.state.build_up_mode}\n"
            f"- build_up_mode_stdin_value_to_send -> {build_up_answer}\n"
            "- known_prompt_4_when_build_up -> Prompt interaction mode [1=Interactive]:\n"
            ...
            "- known_prompt_5_when_build_up -> Philosophy lens [1=Balanced / Unknown]:\n"
            f"- ui_philosophy_lens -> {self.state.selected_philosophy}\n"
            f"- philosophy_lens_stdin_value_to_send -> {philosophy_answer}\n"
            f"- philosophy_lens_bridge_active_for_this_direction -> {self.should_send_philosophy_lens_input()}\n"
            "- specific_philosophy_subtype_answered -> False; top-level lens only in this patch\n"
            "- known_prompt_6_when_build_up -> Guide presentation [3=Either/random]:\n"
            f"- ui_guide_presentation -> {self.state.guide_presentation}\n"
            f"- guide_presentation_stdin_value_to_send -> {guide_answer}\n"
            f"- guide_presentation_bridge_active_for_this_direction -> {self.should_send_guide_presentation_input()}\n"
            "- known_prompt_7_when_build_up -> Collection mode [1=No]:\n"
            f"- ui_collection_mode -> {self.state.collection_mode}\n"
            f"- collection_mode_stdin_value_to_send -> {collection_answer}\n"
            f"- collection_mode_bridge_active_for_this_direction -> {self.should_send_collection_mode_input()}\n"
            "- known_prompt_8_when_collection_enabled -> Collection source [1=Entire collection folder]:\n"
            ...
            "- collection_folder_or_file_path_answered -> False; folder/file path follow-up prompts are intentionally unmapped in this patch\n"
            "- stdin_closed_after_known_answers -> True\n"
            "- unknown_future_prompts_answered -> False\n"
            "- purpose -> advance known CLI prompts safely, then capture the next prompt/error for mapping review\n"
    ...
    def guarded_execution_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.8 sends known CLI answers through Collection Source when collection is enabled in the Build-up flow.\n"
            "- For Cut down and Auto batch, later direction-specific prompts are not guessed yet.\n"
            "- After sending known answers, stdin is closed so unknown later prompts are captured safely instead of guessed.\n\n"
            ...
            f"- collection_folder_or_file_path_answered -> False\n"
            f"- specific_philosophy_subtype_answered -> False\n"
            ...
            "- cli_input_bridge -> active for output mode, review direction, build-up mode when applicable, prompt mode when safe, top-level philosophy lens when safe, guide presentation when safe, collection mode when safe, and collection source when collection is enabled\n"
    ...
    def guarded_execution_placeholder_message(self):
        QMessageBox.information(
            ...
            "v0.6.7.9.8 can attempt a guarded QProcess run of py main.py only after explicit confirmation.\n\n"
            "This patch sends Output Mode, Review Direction, Build-Up Mode when Review Direction is Build up, Prompt Mode, the top-level Philosophy Lens, Guide Presentation, Collection Mode, and Collection Source when collection is enabled in the known Build-up flow, then safely captures the next prompt/error for review."
        )
    ...
    def start_guarded_backend_run(self):
        ...
        message = (
            ...
            "- Specific philosophy subtypes are not answered yet because the UI currently exposes only the top-level profiles.\n"
            "- Collection folder/file path follow-up prompts are not answered yet.\n"
        ...
        self.backend_process_timed_out = False
        self.guarded_cli_input_sent = False
        self.state.cli_input_bridge_last_sent = "Pending output-mode, review-direction, optional build-up-mode, safe prompt-mode, safe philosophy-lens, safe guide-presentation, safe collection-mode, and conditional collection-source inputs..."
        self.refresh_run_analysis_previews()
    ...
    def send_known_cli_inputs_to_guarded_process(self):
        ...
        input_lines = [output_value, direction_value]
        sent_parts = [
        ...
        if self.state.review_direction == "Build up":
            ...
            sent_parts.append("build-up-mode answer not sent because UI Review Direction is not Build up")
        ...
        if self.should_send_philosophy_lens_input():
            ...
            sent_parts.append("philosophy-lens answer not sent until prior prompts are mapped for this review direction")
        if self.should_send_guide_presentation_input():
            ...
            sent_parts.append("guide-presentation answer not sent until philosophy lens is safely bridged for this review direction")
        if self.should_send_collection_mode_input():
            ...
            sent_parts.append("collection-mode answer not sent until guide presentation is safely bridged for this review direction")
    ...
    def backend_runtime_config_boundary_text(self):
        return (
            "v0.6.7.9.7 boundary\n"
            "- This page now includes a guarded execution bridge with controlled stdin inputs through Collection Mode for the known Build-up flow.\n"
            ...
            "- This patch answers only known top-level prompts through Collection Mode. Collection source/folder/file follow-up prompts, specific philosophy subtypes, and later prompts should be mapped deliberately instead of guessed."
        )
    ...
    def page_run_review(self):
        ...
        boundary_card = ReportCard("Safety Boundary and Future Stages", self.theme, badges=[("v0.6.7.9.7", "manual")])
        stage_text = (
