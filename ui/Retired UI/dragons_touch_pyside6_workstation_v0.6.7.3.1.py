...
APP_VERSION = "v0.6.7.3.1"
APP_PHASE = "Review Setup Dropdown and Defaults Cleanup"
...
# - v0.6.7.3.1 cleans up dropdown readability, default labels, and review setup handoff wording.
...
class MainWindow(QMainWindow):
    ...
    def qss(self, t):
        ...
        QComboBox QAbstractItemView {{ color: {combo_popup_text}; background: {combo_popup_bg}; border: 1px solid {t["border"]}; selection-background-color: {combo_popup_selected_bg}; selection-color: {combo_popup_selected_text}; outline: 0; padding: 4px; }}
    ...
    def page_deck_input(self):
        ...
        decklist.setMinimumHeight(180)
        decklist.setMaximumHeight(205)
        decklist.setFixedHeight(195)
    ...
    def page_analysis_setup(self):
        page, layout = self.page_container(
            "Review Setup",
            f"Stage the same review choices the CLI already supports. {APP_VERSION} clarifies defaults, dropdowns, and collection handoff before backend mapping."
        )
        ...
        output_combo = QComboBox(); output_combo.addItems(["Normal User Mode", "Debug / Stress-Test Mode", "Both"]); output_combo.setCurrentText(self.state.output_mode); output_combo.setMaxVisibleItems(8)
        ...
        direction_combo = QComboBox(); direction_combo.addItems(["Build up", "Cut down", "Auto batch"]); direction_combo.setCurrentText(self.state.review_direction); direction_combo.setMaxVisibleItems(8)
        ...
        cut_combo = QComboBox(); cut_combo.addItems(["Light", "Normal", "Strict", "Brutal / Deep Review", "Rebuild"]); cut_combo.setCurrentText(self.state.cut_depth); cut_combo.setMaxVisibleItems(8)
        ...
        prompt_combo = QComboBox(); prompt_combo.addItems(["Interactive AI chat", "One-shot worksheet"]); prompt_combo.setCurrentText(self.state.prompt_mode); prompt_combo.setMaxVisibleItems(8)
    ...
    def page_settings(self):
        ...
        version = ReportCard("App Version", self.theme); version.body.addWidget(self.make_text(f"The Dragon’s Touch PySide6 Workstation\nVersion: {APP_VERSION}\nPhase: {APP_PHASE}\nLocked backend: {LOCKED_BACKEND_VERSION}\nBackend: Not connected in UI yet\nPurpose: local deck-file selection plus review-settings form staging, dropdown readability, default labels, and collection handoff clarity", paper=True)); b_layout.addWidget(version); b_layout.addStretch(1); layout.addWidget(body, stretch=1); return page
