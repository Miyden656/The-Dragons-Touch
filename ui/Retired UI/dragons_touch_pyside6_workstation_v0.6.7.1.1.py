...
Version: v0.6.7.1.1 — Adventurer's Map Contrast / Readability Cleanup
...
- setup pages are visual/placeholder only
...
APP_VERSION = "v0.6.7.1.1"
APP_PHASE = "Adventurer's Map Contrast / Readability Cleanup"
BACKEND_STATUS = "Backend not connected — CLI remains stable source of truth"
LOCKED_BACKEND_VERSION = "v0.6.6.6"

# Future backend integration notes:
# - v0.6.7.2 should add real deck-file selection.
# - v0.6.7.3 should map UI controls into the existing runtime config.
...
DRAGON_FORGE = {
    ...
    "panel_text": "#f4e8d4", "sidebar_text": "#f4e8d4", "sidebar_muted": "#bca892",
    "sidebar_hover_text": "#fff2d6", "sidebar_checked_text": "#100d0b",
...
ADVENTURERS_MAP = {
    ...
    "panel_text": "#3a2818", "sidebar_text": "#fff0cf", "sidebar_muted": "#f3ddad",
    "sidebar_hover_text": "#fff7dc", "sidebar_checked_text": "#2b1b0d",
    "button_text": "#2b1b0d", "button_pressed_text": "#fff8e3",
    "input_text": "#2b1b0d", "progress_text": "#2b1b0d",
    "progress_track": "#f7e7bf", "progress_chunk_start": "#3e6b3a", "progress_chunk_end": "#b88a2e",
}
...
class AppState:
    ...
    warnings: int = 0
    status: str = "UI foundation ready"
...
class MainWindow(QMainWindow):
    ...
    def qss(self, t):
        ...
        QPushButton:pressed {{ background: {t["accent"]}; color: {button_pressed_text}; }} QPushButton#primaryButton {{ color: {sidebar_checked_text}; border: 1px solid {t["accent_2"]}; background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 {t["accent"]}, stop:1 {t["accent_2"]}); font-weight: 900; }}
        ...
        QPushButton#sidebarButton:hover {{ color: {sidebar_hover_text}; background: {t["sidebar_2"]}; border: 1px solid {t["accent_2"]}; }} QPushButton#sidebarButton:checked {{ color: {sidebar_checked_text}; background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 {t["accent"]}, stop:1 {t["accent_2"]}); border: 1px solid {t["accent_2"]}; font-weight: 900; }}
        QPushButton#pillButton {{ border-radius: 15px; padding: 7px 12px; color: {sidebar_muted}; background: {t["iron_2"]}; border: 1px solid {t["border_soft"]}; }} QPushButton#pillButton:checked {{ color: {sidebar_checked_text}; background: {t["accent"]}; border: 1px solid {t["accent_2"]}; }}
    ...
    def page_deck_input(self):
        page, layout = self.page_container("Deck Selection", "Place the deck on the forge-table. v0.6.7.1 keeps selection visual only; real file loading comes in v0.6.7.2.")
        ...
        for label in ["Choose Deck File", "Preview Deck", "Paste Decklist Later"]:
        ...
        quick = ReportCard("Forge Note", self.theme); quick.body.addWidget(self.make_text("v0.6.7.1 establishes the desktop shell. v0.6.7.2 should connect file selection and deck preview while leaving the locked CLI backend untouched.", paper=True))
    ...
    def page_analysis_setup(self):
        page, layout = self.page_container("Review Setup", "Set the same review choices the CLI already supports: output mode, review direction, cut depth, prompt mode, and table-fit boundaries.")
        ...
        settings = [("Output Mode", ["Normal User Mode", "Debug / Stress-Test Mode", "Both"], "Both"), ("Review Direction", ["Build up", "Cut down", "Auto batch"], "Cut down"), ("Cut Depth", ["Light", "Normal", "Strict", "Brutal / Deep Review", "Rebuild"], "Normal"), ("Prompt Mode", ["Interactive AI chat", "One-shot worksheet"], "Interactive AI chat")]
        ...
        budget_card = ReportCard("Table / Budget Boundaries", self.theme); budget_card.body.addWidget(QLineEdit("Optional budget note, e.g. $25/card")); budget_card.body.addWidget(QCheckBox("Respect stated table bracket")); budget_card.body.addWidget(QCheckBox("Use collection settings when connected")); grid.addWidget(budget_card, 2, 0)
    ...
    def page_run_review(self):
        page, layout = self.page_container("Run Analysis", "Prepare the future backend run. v0.6.7.1 only shows the analysis workflow; it does not call the engine yet.")
        ...
        status = QLabel("UI foundation status: ready. Backend run hook intentionally not connected in v0.6.7.1."); status.setObjectName("helperText"); status.setAlignment(Qt.AlignCenter); status.setWordWrap(True); l_layout.addWidget(status)
    ...
    def page_report_viewer(self):
        ...
        intro = QLabel("Prepared for future Dragon’s Touch reports. v0.6.7.1 demonstrates the intended long-form reading experience. Later patches will load normal reports, debug reports, and aggregate reports from the output folder."); intro.setObjectName("reportBody"); intro.setWordWrap(True); m_layout.addWidget(intro)
    ...
    def page_collection_tools(self):
        ...
        import_card = ReportCard("Collection Source Selection", self.theme, badges=[("v0.6.7.4 later", "manual")]); import_card.body.addWidget(self.make_text("Future behavior: choose entire collection folder or select multiple collection files. v0.6.7.1 keeps this visual only.", paper=True)); import_card.body.addWidget(QPushButton("Choose Collection Source Later")); source = QComboBox(); source.addItems(["No collection", "Prefer collection first", "Collection only", "Collection shakeup"]); import_card.body.addWidget(source)
