...
Version: v0.6.7.9.4 — Prompt Mode CLI Bridge and Direction-Aware Review Setup UI
...
APP_VERSION = "v0.6.7.9.4"
APP_PHASE = "Prompt Mode CLI Bridge and Direction-Aware Review Setup UI"
BACKEND_STATUS = "Guarded backend bridge available — output, review direction, build-up, and prompt mode CLI bridge active"
...
class AppState:
    ...
    cli_input_bridge_scope: str = "Output mode, review direction, build-up mode when applicable, and prompt mode when safe"
...
class MainWindow(QMainWindow):
    ...
    def page_analysis_setup(self):
        ...
        cut_card.body.addWidget(self.default_note("Used when Review Direction is Cut down. Also used as an Auto batch default.")); grid.addWidget(cut_card, 1, 0)
        ...
        grid.addWidget(build_up_card, 2, 0)
        ...
        grid.addWidget(budget_card, 3, 0)
        ...
        grid.addWidget(summary, 3, 1)

        def refresh_direction_specific_review_fields():
            direction = direction_combo.currentText()
            cut_card.setVisible(direction in {"Cut down", "Auto batch"})
            build_up_card.setVisible(direction in {"Build up", "Auto batch"})
        ...
        n_title = QLabel("v0.6.7.9.4 Boundary"); n_title.setObjectName("sectionTitle"); n_layout.addWidget(n_title)
        n_layout.addWidget(self.make_text("These choices auto-stage inside the UI as soon as you change them. Review Setup now shows Review Intensity for Cut down, Build-Up Mode for Build up, and both for Auto batch. Auto batch remains development-oriented until the workflow is fully automated."))
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.4 purpose: add direction-aware Review Setup display and bridge the known Prompt Mode CLI prompt for the Build-up flow"
        )
    ...
    def cli_input_bridge_preview_text(self):
        ...
        return (
            "Prompt Mode CLI Input Bridge\n"
            "- bridge_scope -> Output mode, review direction, build-up mode when needed, and prompt mode when the known CLI path reaches it\n"
    ...
    def guarded_execution_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.4 sends known CLI answers through Prompt Mode for the Build-up flow.\n"
            ...
            "- cli_input_bridge -> active for output mode, review direction, build-up mode when applicable, and prompt mode when safe\n"
    ...
    def guarded_execution_placeholder_message(self):
        QMessageBox.information(
            ...
            "v0.6.7.9.4 can attempt a guarded QProcess run of py main.py only after explicit confirmation.\n\n"
            "This patch sends Output Mode, Review Direction, Build-Up Mode when Review Direction is Build up, and Prompt Mode when the known Build-up flow reaches that prompt, then safely captures the next prompt/error for review."
        )
    ...
    def start_guarded_backend_run(self):
        ...
        self.backend_process_timed_out = False
        self.guarded_cli_input_sent = False
        self.state.cli_input_bridge_last_sent = "Pending output-mode, review-direction, optional build-up-mode, and safe prompt-mode inputs..."
        self.refresh_run_analysis_previews()
    ...
    def page_run_review(self):
        ...
        boundary_card = ReportCard("Safety Boundary and Future Stages", self.theme, badges=[("v0.6.7.9.4", "manual")])
        stage_text = (
