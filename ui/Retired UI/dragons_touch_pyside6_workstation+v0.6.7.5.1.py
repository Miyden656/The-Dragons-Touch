...
Version: v0.6.7.5.1 — Staged Update Refresh / Popup Behavior Cleanup
...
APP_VERSION = "v0.6.7.5.1"
APP_PHASE = "Staged Update Refresh / Popup Behavior Cleanup"
...
# - v0.6.7.5.1 cleans up staged refresh timing so summaries update before confirmation popups.
...
class AppState:
    ...
    warnings: int = 0
    status: str = "Run analysis prep ready"
...
class MainWindow(QMainWindow):
    ...
    def page_analysis_setup(self):
        ...
        n_layout.addWidget(self.make_text("These choices are now real UI state. They are not yet handed to the locked backend. Collection mode belongs on the Collection Source page. v0.6.7.5.1 keeps staged refresh behavior clean before runtime-config mapping."))
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.5.1 purpose: confirm staged information and clean up refresh/popup timing before a future backend bridge"
        )
    ...
    def page_run_review(self):
        ...
        stages_card = ReportCard("Future Backend Mapping Stages", self.theme, badges=[("v0.6.7.5.1", "manual")])
        stage_text = (
        ...
        b_title = QLabel("v0.6.7.5.1 Boundary"); b_title.setObjectName("sectionTitle"); b_layout.addWidget(b_title)
        b_layout.addWidget(self.make_text("This patch keeps the backend hook preview in one place and cleans up staged UI refresh timing. It does not run analysis, load Scryfall, validate legality, load a collection, or write reports."))
