...
Version: v0.6.7.9.9 — Full CLI Bridge Gap Pass
...
APP_VERSION = "v0.6.7.9.9"
APP_PHASE = "Full CLI Bridge Gap Pass"
BACKEND_STATUS = "Guarded backend bridge available — full known CLI bridge gap pass active"
...
class MainWindow(QMainWindow):
    ...
    def page_philosophy(self):
        ...
        subtype_card.body.addWidget(self.default_note("Default: None / top-level only. Specific subtype numbering is bridged as a best-effort mirror of the planned subtype order."))
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.9 purpose: attempt the full known CLI bridge gap pass across build-up, cut-down, auto-batch defaults, philosophy subtype, guide presentation, collection mode, and collection source"
        )
    ...
    def guarded_execution_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.9 attempts all known CLI bridge gaps across Build-up, Cut-down, Auto-batch defaults, philosophy subtype, guide presentation, collection mode, and collection source.\n"
            ...
            f"- collection_folder_or_file_path_answered -> {self.state.collection_source_mode == 'Select collection files' and bool(self.state.selected_collection_files)}\n"
    ...
    def guarded_execution_placeholder_message(self):
        QMessageBox.information(
            ...
            "This patch attempts the full known CLI bridge gap pass across Build-up, Cut-down, Auto-batch defaults, Philosophy Lens/subtype, Guide Presentation, Collection Mode, and Collection Source, then safely captures any unexpected prompt/error for review."
        )
    ...
    def start_guarded_backend_run(self):
        ...
        message = (
            ...
            "- Collection folder/file path follow-up prompts are only answered when selected files are explicitly staged; unexpected path prompts are captured safely.\n"
    ...
    def send_known_cli_inputs_to_guarded_process(self):
        ...
        if self.should_send_collection_source_input():
            ...
            # Conservative best-effort: only send typed selected file paths when the UI has explicit files.
            # Entire-folder mode currently works with the backend's default folder and does not need a typed path.
            if self.state.collection_source_mode == "Select collection files" and self.state.selected_collection_files:
    ...
    def backend_runtime_config_boundary_text(self):
        return (
            "v0.6.7.9.9 boundary\n"
            "- This page now includes a guarded execution bridge with controlled stdin inputs across the full known CLI bridge gap pass.\n"
            ...
            "- This patch attempts all known bridge gaps in one pass. If any backend prompt differs from the expected order, the UI captures stdout/stderr safely so the bridge can be corrected without hiding the failure."
        )
