...
Version: v0.6.7.9.3 — Build-Up Mode UI Field and CLI Input Bridge
...
APP_VERSION = "v0.6.7.9.3"
APP_PHASE = "Build-Up Mode UI Field and CLI Input Bridge"
BACKEND_STATUS = "Guarded backend bridge available — output mode, review direction, and build-up mode CLI bridge active"
...
# - v0.6.7.9.3 adds the second controlled stdin bridge for the known review-direction prompt only.
...
class AppState:
    ...
    cli_input_bridge_scope: str = "Output mode, review direction, and build-up mode prompt when applicable"
...
class MainWindow(QMainWindow):
    ...
    def page_analysis_setup(self):
        ...
        build_up_card.body.addWidget(self.default_note("Used when Review Direction is Build up. Staged now so the backend bridge does not overload Review Intensity."))
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.3 purpose: add the second controlled stdin bridge for main.py by sending output mode and review direction answers, then safely capturing the next prompt/error"
        )
    ...
    def cli_input_bridge_preview_text(self):
        ...
        return (
            "Build-Up Mode CLI Input Bridge\n"
            "- bridge_scope -> Output mode, review direction, and build-up mode when Review Direction is Build up\n"
            ...
            "- known_prompt_3 -> Build-up mode [4=Finalize]:\n"
            f"- ui_build_up_mode -> {self.state.build_up_mode}\n"
            f"- build_up_mode_stdin_value_to_send -> {build_up_answer}\n"
            "- stdin_lines_sent -> two values normally; three values only for Build up\n"
            "- stdin_closed_after_known_answers -> True\n"
            "- additional_prompts_after_build_up_mode_answered -> False\n"
    ...
    def guarded_execution_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.3 sends only two known CLI answers: Output Mode and Review Direction.\n"
            ...
            "- cli_input_bridge -> active for output mode, review direction, and build-up mode when applicable\n"
    ...
    def guarded_execution_placeholder_message(self):
        QMessageBox.information(
            ...
            "v0.6.7.9.3 can attempt a guarded QProcess run of py main.py only after explicit confirmation.\n\n"
            "This patch sends Output Mode and Review Direction, plus Build-Up Mode when Review Direction is Build up, then safely captures the next prompt/error for review."
        )
    ...
    def start_guarded_backend_run(self):
        ...
        self.backend_process_timed_out = False
        self.guarded_cli_input_sent = False
        self.state.cli_input_bridge_last_sent = "Pending output-mode, review-direction, and optional build-up-mode inputs..."
        self.refresh_run_analysis_previews()
