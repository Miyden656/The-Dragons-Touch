...
Version: v0.6.7.9.7 — Collection Mode CLI Bridge
...
APP_VERSION = "v0.6.7.9.7"
APP_PHASE = "Collection Mode CLI Bridge"
BACKEND_STATUS = "Guarded backend bridge available — output, review direction, build-up, prompt mode, philosophy, guide, and collection mode CLI bridge active"
...
class AppState:
    ...
    cli_input_bridge_scope: str = "Output mode, review direction, build-up mode when applicable, prompt mode when safe, top-level philosophy lens when safe, guide presentation when safe, and collection mode when safe"
...
class MainWindow(QMainWindow):
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.7 purpose: bridge Collection Mode after Guide Presentation while keeping collection source/folder/file follow-up prompts staged for later mapping"
        )
    ...
    def cli_input_bridge_preview_text(self):
        ...
        return (
            "Collection Mode CLI Input Bridge\n"
            "- bridge_scope -> Output mode, review direction, build-up mode when needed, prompt mode when safe, top-level philosophy lens when safe, guide presentation when safe, and collection mode when safe\n"
            ...
            "- collection_source_details_answered -> False; source/folder/file follow-up prompts are intentionally unmapped in this patch\n"
    ...
    def guarded_execution_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.7 sends known CLI answers through Collection Mode for the Build-up flow.\n"
            ...
            f"- collection_source_followup_answered -> False\n"
            ...
            "- cli_input_bridge -> active for output mode, review direction, build-up mode when applicable, prompt mode when safe, top-level philosophy lens when safe, guide presentation when safe, and collection mode when safe\n"
    ...
    def guarded_execution_placeholder_message(self):
        QMessageBox.information(
            ...
            "v0.6.7.9.7 can attempt a guarded QProcess run of py main.py only after explicit confirmation.\n\n"
            "This patch sends Output Mode, Review Direction, Build-Up Mode when Review Direction is Build up, Prompt Mode, the top-level Philosophy Lens, Guide Presentation, and Collection Mode when the known Build-up flow reaches those prompts, then safely captures the next prompt/error for review."
        )
    ...
    def start_guarded_backend_run(self):
        ...
        message = (
            ...
            "- Collection source/folder/file follow-up prompts are not answered yet.\n"
        ...
        self.backend_process_timed_out = False
        self.guarded_cli_input_sent = False
        self.state.cli_input_bridge_last_sent = "Pending output-mode, review-direction, optional build-up-mode, safe prompt-mode, safe philosophy-lens, safe guide-presentation, and safe collection-mode inputs..."
        self.refresh_run_analysis_previews()
