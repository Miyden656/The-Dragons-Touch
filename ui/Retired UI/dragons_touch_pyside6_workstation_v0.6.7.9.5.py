...
Version: v0.6.7.9.5 — Philosophy Lens CLI Bridge and Review Setup Conditional Layout Cleanup
...
APP_VERSION = "v0.6.7.9.5"
APP_PHASE = "Philosophy Lens CLI Bridge and Review Setup Conditional Layout Cleanup"
BACKEND_STATUS = "Guarded backend bridge available — output, review direction, build-up, prompt mode, and philosophy CLI bridge active"
...
# - v0.6.7.9.5 adds Philosophy Lens CLI bridging and cleans up Review Setup conditional card layout.
...
class AppState:
    ...
    cli_input_bridge_scope: str = "Output mode, review direction, build-up mode when applicable, prompt mode when safe, and top-level philosophy lens when safe"
...
class MainWindow(QMainWindow):
    ...
    def page_analysis_setup(self):
        ...
        n_title = QLabel("v0.6.7.9.5 Boundary"); n_title.setObjectName("sectionTitle"); n_layout.addWidget(n_title)
    ...
    def run_config_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.5 purpose: clean up conditional Review Setup layout and bridge the top-level Philosophy Lens CLI prompt for the Build-up flow"
        )
    ...
    def cli_input_bridge_preview_text(self):
        ...
        return (
            "Philosophy Lens CLI Input Bridge\n"
            "- bridge_scope -> Output mode, review direction, build-up mode when needed, prompt mode when safe, and top-level philosophy lens when safe\n"
    ...
    def guarded_execution_preview_text(self):
        ...
        return (
            ...
            "- v0.6.7.9.5 sends known CLI answers through top-level Philosophy Lens for the Build-up flow.\n"
            ...
            "- cli_input_bridge -> active for output mode, review direction, build-up mode when applicable, prompt mode when safe, and top-level philosophy lens when safe\n"
    ...
    def guarded_execution_placeholder_message(self):
        QMessageBox.information(
            ...
            "v0.6.7.9.5 can attempt a guarded QProcess run of py main.py only after explicit confirmation.\n\n"
            "This patch sends Output Mode, Review Direction, Build-Up Mode when Review Direction is Build up, Prompt Mode, and the top-level Philosophy Lens when the known Build-up flow reaches those prompts, then safely captures the next prompt/error for review."
        )
    ...
    def start_guarded_backend_run(self):
        ...
        self.backend_process_timed_out = False
        self.guarded_cli_input_sent = False
        self.state.cli_input_bridge_last_sent = "Pending output-mode, review-direction, optional build-up-mode, safe prompt-mode, and safe philosophy-lens inputs..."
        self.refresh_run_analysis_previews()
    ...
    def page_run_review(self):
        ...
        boundary_card = ReportCard("Safety Boundary and Future Stages", self.theme, badges=[("v0.6.7.9.5", "manual")])
        stage_text = (
