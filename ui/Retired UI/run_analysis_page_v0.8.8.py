...
def build_run_analysis_page(window):
    ...
    page, layout = window.page_container(
        "Run Analysis",
        f"Run the selected deck through the guarded backend. {APP_VERSION} keeps advanced diagnostics available, but the main action is Run Analysis."
    )
    ...
    result_card = ReportCard("Run Output / Result", window.theme, badges=[("Captured", "manual"), ("Review after run", "protected")])
