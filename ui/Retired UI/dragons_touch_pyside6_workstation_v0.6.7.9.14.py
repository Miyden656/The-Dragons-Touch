...
Version: v0.6.7.9.11 — Collection Folder Handoff State Cleanup
...
APP_VERSION = "v0.6.7.9.14"
APP_PHASE = "Guarded Run Report Output Detection"
...
class MainWindow(QMainWindow):
    ...
    def detect_report_outputs_from_stdout(self, stdout_text):
        """Parse the backend Files written block and store output paths/folders without reading report contents."""
        ...
        for line in lines:
            stripped = line.strip()
            if stripped.lower().startswith("files written"):
                in_files_block = True
                continue
            if in_files_block:
                if stripped.startswith("- "):
                    resolved = self.resolve_backend_output_path(stripped[2:].strip())
                    if resolved is not None:
                        detected.append(str(resolved))
                elif stripped == "":
                    continue
                elif detected:
                    break
        self.state.last_output_files = detected
        self.state.last_normal_report_files = [p for p in detected if Path(p).parent.name.lower() == "normal"]
        self.state.last_debug_report_files = [p for p in detected if Path(p).parent.name.lower() == "debug"]
        if detected:
            first_parent = Path(detected[0]).parent
            self.state.last_output_folder = str(first_parent.parent if first_parent.name.lower() in {"normal", "debug"} else first_parent)
        ...
        if detected:
            self.state.last_report_detection_status = f"Detected {len(detected)} output file(s): {len(self.state.last_normal_report_files)} normal, {len(self.state.last_debug_report_files)} debug."
        else:
            self.state.last_report_detection_status = "No 'Files written:' report paths detected in guarded-run stdout."

    def report_output_summary_text(self):
        normal_preview = "None detected"
        if self.state.last_normal_report_files:
            normal_preview = "\n".join(f"  - {Path(p).name}" for p in self.state.last_normal_report_files[:6])
            if len(self.state.last_normal_report_files) > 6:
                normal_preview += f"\n  - ...and {len(self.state.last_normal_report_files) - 6} more"
        debug_preview = "None detected"
        if self.state.last_debug_report_files:
            debug_preview = "\n".join(f"  - {Path(p).name}" for p in self.state.last_debug_report_files[:8])
            if len(self.state.last_debug_report_files) > 8:
                debug_preview += f"\n  - ...and {len(self.state.last_debug_report_files) - 8} more"
        return (
            ...
            f"{debug_preview}\n\n"
            "Boundary\n"
            "- The UI detects file paths from stdout only.\n"
    ...
    def finish_guarded_backend_run(self, exit_code, exit_status):
        ...
        if self.backend_process_timed_out:
            ...
            self.state.last_guarded_run_status = "Completed successfully. Report output paths were detected when available; review stdout or open detected folders below."
        else:
    ...
    def backend_runtime_config_boundary_text(self):
        return (
            ...
            "- This patch detects report file paths from the backend Files written stdout block after successful guarded runs and exposes safe local folder-open buttons without parsing report contents."
        )
    ...
    def page_run_review(self):
        ...
        boundary_card = ReportCard("Safety Boundary and Future Stages", self.theme, badges=[("v0.6.7.9.14", "manual")])
        stage_text = (
