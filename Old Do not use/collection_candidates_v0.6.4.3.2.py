...
v0.6.4.3.1 scope:
- Tighten owned-card candidate quality gates.
- Prevent broad / accidental role overlaps from becoming "Strong" recommendations.
- Keep "shakeup" candidates clearly separate from real upgrade suggestions.
...
def _category_role_needs(category: str) -> CategoryNeed:
    ...
    if "evasion" in cat or "trample" in cat:
        return CategoryNeed(frozenset({"evasion_support"}), frozenset({"combat_support", "finisher_or_payoff"}), "evasion/trample")
    if "combat finisher" in cat or "finisher" in cat:
        return CategoryNeed(frozenset({"finisher_or_payoff", "evasion_support"}), frozenset({"combat_support", "token_production", "creature"}), "combat finisher")
    if "combat payoff" in cat or "combat" in cat:
        return CategoryNeed(frozenset({"finisher_or_payoff", "evasion_support", "token_production"}), frozenset({"combat_support", "creature"}), "combat payoff")
...
def build_collection_candidate_summary(
    ...
    summary.quality_gate_notes.extend([
        "Strong candidates require a direct need match plus strategy or role relevance.",
    ...
    for card_name, quantity in (getattr(collection_summary, "card_quantities", {}) or {}).items():
        ...
        for category, need in category_needs.items():
            ...
            if direct_overlap:
                ...
                # Support-only overlap can make a card possible, but it should not
                # satisfy the category strongly enough to erase the collection gap.
                matched_categories.append(category)
                any_category_hits[category] += 1
        ...
        if strategy_overlap:
            score += 2 if strategy_overlap_roles - BROAD_ROLES else 1
        ...
        if direct_hits == 0 and support_hits > 0:
            ...
            quality_gate = "support-only overlap; not enough for a strong upgrade"
            downgrade_reasons["support_only"] += 1
        if not matched_categories and strategy_overlap:
            cap_to_shakeup = True
            quality_gate = "strategy-adjacent but does not match a current deck need"
        ...
        if _is_generic_colorless_artifact(card, role_set):
            ...
            warnings.append(msg)
            quality_gate = msg if not quality_gate else f"{quality_gate}; {msg}"
        ...
        if not strategy_overlap and direct_hits <= 1 and matched_categories:
            cap_to_possible = True
            msg = "matches a role need but has weak strategy overlap"
            quality_gate = msg if not quality_gate else f"{quality_gate}; {msg}"
            downgrade_reasons["weak_strategy_overlap"] += 1

        if not quality_gate:
            quality_gate = "direct need match plus strategy/role relevance"

        source_files = _source_files_for_card(collection_summary, card_name)
        reason = _candidate_reason(matched_categories, roles, strategy_overlap, quality_gate)

        if matched_categories and direct_hits >= 1 and score >= 7 and not warnings and not cap_to_possible and not cap_to_shakeup:
            for category in matched_categories:
                strong_category_hits[category] += 1
            candidate = _make_candidate(card_name, quantity, "Strong", "Owned card directly supports a current deck need", matched_categories, roles, reason, card, source_files, quality_gate=quality_gate)
