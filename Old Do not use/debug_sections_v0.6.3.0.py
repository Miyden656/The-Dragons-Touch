...
def build_diagnostics_debug_section(context: dict[str, Any]) -> str:
    ...
    lines.extend([
        ...
        "- Full decklist export expected in normal report: Yes",
        f"- Main deck card entries available to report: {len(getattr(parsed, 'cards', []) or [])}",
        f"- Unique counted cards available to report: {len(getattr(parsed, 'unique_cards', {}) or {})}",
        f"- Main deck sections available to report: {len(cards_by_section)}",
        f"- Reference/non-mainboard card entries available to report: {len(getattr(parsed, 'reference_cards', []) or [])}",
        f"- Reference/non-mainboard sections available to report: {len(reference_by_section)}",
        "",
        "## Parser hygiene",
