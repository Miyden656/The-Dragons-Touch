...
def render_philosophy_prompt_questions(context: dict) -> str:
    """Render questions for the user-guided prompt without changing the old seven-section flow."""
    guide = context.get("guide_name") or context.get("label", "The selected philosophy")
    key = context.get("key", "balanced_unknown")

    base = [
        "### Philosophy Guide Add-On",
