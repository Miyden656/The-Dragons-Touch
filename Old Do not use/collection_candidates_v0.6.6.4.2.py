...
def _replacement_bias_matches(role_set: set[str], philosophy_context: dict[str, Any] | None) -> list[str]:
    ...
    v0.6.6.4.2 uses these matches as a light presentation/order nudge only.
...
def build_collection_candidate_summary(
    ...
    summary.quality_gate_notes.extend([
        ...
        "v0.6.6.4.2 replacement bias visibility is active: philosophy nudges are counted, exampled, and still cannot override quality gates.",
    ])
    ...
    if summary.replacement_bias_active:
        if summary.replacement_bias_adjusted_cards:
            summary.notes.append(f"v0.6.6.4.2 philosophy-aware replacement bias nudged {summary.replacement_bias_adjusted_cards} owned candidate(s) for {summary.replacement_bias_lens}; this is not an automatic upgrade verdict.")
        else:
            summary.notes.append(f"v0.6.6.4.2 replacement bias was active for {summary.replacement_bias_lens}, but no owned candidates had enough normal deck-fit evidence to receive a philosophy nudge.")
