...
def _ordered_count_lines(cards: list[str]) -> list[str]:
    """Return counted decklist lines in first-seen order."""
    if not cards:
        return []
    counts: dict[str, int] = {}
    order: list[str] = []
    for card in cards:
        if card not in counts:
            ...
            order.append(card)
        counts[card] += 1
    return [f"{counts[card]} {card}" for card in order]


def _render_counted_text_block(lines: list[str], empty: str = "None detected.") -> list[str]:
    if not lines:
        return [empty]
    return ["```text", *lines, "```"]


def _format_sectioned_decklist(parsed: Any) -> list[str]:
    """Render the parsed decklist by section, preserving parser section order where possible."""
    cards_by_section = getattr(parsed, "cards_by_section", {}) or {}
    if not cards_by_section:
        return _render_counted_text_block(_ordered_count_lines(list(getattr(parsed, "cards", []) or [])))

    lines: list[str] = []
    for section_name, section_cards in cards_by_section.items():
        if not section_cards:
            continue
        lines.append(f"### {section_name}")
        lines.extend(_render_counted_text_block(_ordered_count_lines(list(section_cards))))
        lines.append("")
    return lines or _render_counted_text_block(_ordered_count_lines(list(getattr(parsed, "cards", []) or [])))


def _format_reference_cards(parsed: Any) -> list[str]:
    """Render reference/non-mainboard/ignored card entries for AI handoff review."""
    reference_by_section = getattr(parsed, "reference_cards_by_section", {}) or {}
    reference_cards = list(getattr(parsed, "reference_cards", []) or [])
    if not reference_by_section and not reference_cards:
    ...
    if reference_by_section:
        for section_name, cards in reference_by_section.items():
            if not cards:
                continue
            lines.append(f"### {section_name}")
            lines.extend(_render_counted_text_block(_ordered_count_lines(list(cards))))
            lines.append("")
    elif reference_cards:
        lines.extend(_render_counted_text_block(_ordered_count_lines(reference_cards)))
...
def build_normal_report(context: dict[str, Any]) -> str:
    ...
    lines.extend([
        "> This section is included so the report can be pasted into another AI or deck-review tool without losing access to the actual card list.",
        "",
        f"- Cards counted by script for deck review: {parsed.deck_card_count}",
        f"- Unique counted cards: {len(parsed.unique_cards)}",
        "",
    ])
    lines.extend(_format_sectioned_decklist(parsed))

    lines += _section("Reference / Non-Mainboard / Ignored Cards")
    lines.extend([
        "> These entries were detected outside the counted deck or ignored for deck-size counting. Review them manually if they are intended as companion, sideboard, maybeboard, attraction, sticker, token, or reference cards.",
        "",
        f"- Reference/non-mainboard card entries detected: {parsed.reference_card_count}",
        "",
    ])
    lines.extend(_format_reference_cards(parsed))
