...
def build_diagnostics_debug_section(context: dict[str, Any]) -> str:
    ...
    if philosophy_context:
        ...
        lines.extend([
            ...
            "- Cut scoring changed by philosophy: Yes — v0.6.6.2.1 light optional-cut bias is active",
            ...
            "- Current philosophy scoring phase: v0.6.6.2.1 Philosophy Bias Visibility / Trigger-Amplifier Cleanup",
            "- Next philosophy scoring phase: v0.6.6.4 Philosophy-Aware Replacement Bias",
            "",
            ...
            "- v0.6.6.2.1 optional-cut bias cleanup active: Yes",
            ...
            "- Foundation boundary: v0.6.6.2.1 applies only small optional-cut nudges and adds bias visibility/trigger-amplifier role support. It does not alter legality, required cuts, strategy detection, collection matching, or replacement scoring.",
            ...
            "",
            "## v0.6.6.2.1 Philosophy Bias Visibility / Trigger-Amplifier Cleanup",
            "- v0.6.6.2.1 optional-cut bias cleanup active: Yes",
            ...
            f"- Bias evaluated cards this run: {(philosophy_context.get('cut_bias_runtime_stats') or {}).get('evaluated', 0)}",
            f"- Bias-applied cards this run: {(philosophy_context.get('cut_bias_runtime_stats') or {}).get('applied', 0)}",
            f"- Protect-side adjustments this run: {(philosophy_context.get('cut_bias_runtime_stats') or {}).get('protected_adjustments', 0)}",
            f"- Review-side adjustments this run: {(philosophy_context.get('cut_bias_runtime_stats') or {}).get('review_adjustments', 0)}",
            f"- Bias no-match cards this run: {(philosophy_context.get('cut_bias_runtime_stats') or {}).get('no_match', 0)}",
            ...
            "- Visibility note: if bias-applied cards is 0, the selected lens was active but no evaluated cut candidate matched the current protect/review role map.",
