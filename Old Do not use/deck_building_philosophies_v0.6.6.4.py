...
v0.6.6.4 turns on a light replacement-side candidate nudge while preserving collection honesty and all quality gates.
"""
...
class PhilosophyProfile:
    ...
    # future cut/replacement logic. In v0.6.6.4 the cut-side profile and a light replacement-side candidate nudge are active.
    ...
    bias_warning: str = "v0.6.6.4 applies light optional-cut and replacement-candidate bias only. It must not override legality, required cuts, pilot-protected cards, color identity, companion restrictions, collection mode, quality gates, or explicit pilot intent."
...
def _build_bias_profile(profile: PhilosophyProfile) -> dict:
    ...
    return {
        ...
        "philosophy_bias_foundation_version": "v0.6.6.4",
...
def render_philosophy_diagnostics_section(context: dict) -> str:
    ...
    lines = [
        ...
        "- Guidance scope: normal report framing and diagnostics; v0.6.6.4 applies light optional-cut and replacement-candidate nudges while preserving legality, strategy, collection mode, and quality gates.",
        f"- v0.6.6.4 philosophy cut/replacement bias active: {'Yes' if context.get('philosophy_bias_foundation_active') else 'No'}",
    ...
    if context.get("philosophy_bias_foundation_active"):
        lines.extend([
            ...
            "- v0.6.6.4 optional-cut and replacement-candidate bias active: Yes",
            f"- Bias profile version: {context.get('philosophy_bias_foundation_version', 'v0.6.6.4')}",
            f"- Bias strength: {context.get('bias_strength', 'guidance')}",
            f"- Bias currently applied to cut scoring: {'Yes' if context.get('cut_bias_scoring_active') else 'No'}",
        f"- Bias currently applied to replacement scoring: {'Yes' if context.get('replacement_bias_scoring_active') else 'No'}",
