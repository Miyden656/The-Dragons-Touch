...
def build_diagnostics_debug_section(context: dict[str, Any]) -> str:
    ...
    if philosophy_context:
        ...
        lines.extend([
            ...
            "- Next philosophy scoring phase: v0.6.6.2 Philosophy-Aware Optional Cut Bias",
            "",
            ...
            f"- Bias profile version: {philosophy_context.get('philosophy_bias_foundation_version', 'v0.6.6.1')}",
            f"- Selected lens bias strength: {philosophy_context.get('bias_strength', 'guidance')}",
            f"- Bias currently applied to cut scoring: {'Yes' if philosophy_context.get('bias_scoring_active') else 'No'}",
            f"- Bias currently applied to replacement scoring: {'Yes' if philosophy_context.get('bias_scoring_active') else 'No'}",
            ...
            "- Foundation boundary: this patch exposes bias data for later cut/replacement logic but does not alter any recommendations by itself.",
            ...
            f"- Bias warning: {philosophy_context.get('bias_warning', 'Bias foundation is diagnostics-only in v0.6.6.1.')}",
            "",
