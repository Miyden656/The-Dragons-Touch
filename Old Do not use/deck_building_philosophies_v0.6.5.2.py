...
def render_philosophy_report_guidance(context: dict) -> str:
    """Render v0.6.5.1 report-facing philosophy guidance.
    ...
    protect = _comma_list(context.get("protect_bias", []), 6)
    review = _comma_list(context.get("review_bias", []), 6)
    replacement = _comma_list(context.get("replacement_bias", []), 6)

    lines = [
        "**v0.6.5.1 Philosophy Guidance:** This section explains how the selected lens should shape the human/AI review language. It does not change legality, deck size, color identity, strategy detection, required cuts, or collection matching yet.",
        "",
        "**How this lens should guide the report:**",
        f"- Protect first: {protect}.",
        f"- Question more carefully: {review}.",
        f"- Prefer replacement framing around: {replacement}.",
...
def render_philosophy_prompt_questions(context: dict) -> str:
    ...
    base = [
        ...
        "",
        "v0.6.5.2 prompt boundary:",
        "Use this philosophy as a review lens only. Strategy detection, legality, deck size, color identity, budget, combo tolerance, companion restrictions, collection mode, and user answers take priority.",
        "Do not write as the guide in first person. Do not roleplay a scene. Introduce the guide/lens once, then continue as a practical deck-review assistant.",
        "",
        "Prompt framing priorities:",
        "1. Reflect the selected philosophy in tone and explanation style.",
        "2. Use the selected philosophy to explain why some cards deserve protection or review.",
        "3. Let pilot corrections override both the script's read and the philosophy lens.",
        "4. Keep all recommendations grounded in the supplied deck report.",
        "",
...
def render_philosophy_diagnostics_section(context: dict) -> str:
    ...
    lines = [
        ...
        "- v0.6.5.2 prompt/persona guidance active when prompt_builder.py is updated: Yes",
        "- Guidance scope: report/prompt framing and diagnostics only; no cut-score or replacement-score changes yet.",
    ]
